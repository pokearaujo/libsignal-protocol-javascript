...
module.exports = function (libsignal = {}, Internal = {}) {
  ...
  SessionCipher.prototype = {
          ...
          return Promise.all([
                ...
                return Internal.crypto
                  ...
                  .then(
                    function (ciphertext) {
                      ...
                      var macInput = new Uint8Array(
                        encodedMsg.byteLength + 33 * 2 + 1
                      );
                      macInput.set(
                        new Uint8Array(
                      ...
                      return Internal.crypto
                        .sign(keys[1], macInput.buffer)
                        .then(
                          function (mac) {
                            var result = new Uint8Array(
                              encodedMsg.byteLength + 9
                            );
                            result[0] = (3 << 4) | 3;
                            result.set(new Uint8Array(encodedMsg), 1);
                            result.set(
                              new Uint8Array(mac, 0, 8),
                              encodedMsg.byteLength + 1
                            );
            ...
            .then(function (message) {
              if (session.pendingPreKey !== undefined) {
                ...
                preKeyMsg.identityKey = util.toArrayBuffer(
                  ourIdentityKey.pubKey
                );
                preKeyMsg.registrationId = myRegistrationId;

                preKeyMsg.baseKey = util.toArrayBuffer(
                ...
                var result =
                  String.fromCharCode((3 << 4) | 3) +
                  util.toString(preKeyMsg.encode());
                return {
                  type: 3,
                  body: result,
    ...
    decryptWhisperMessage: function (buffer, encoding) {
      ...
      return Internal.SessionLock.queueJobForNumber(
        ...
        function () {
          var address = this.remoteAddress.toString();
          return this.getRecord(address).then(
            function (record) {
              if (!record) {
                throw new Error("No record for device " + address);
              }
              var errors = [];
              return this.decryptWithSessionList(
                buffer,
                record.getSessions(),
                errors
              ).then(
      ...
      return Internal.SessionLock.queueJobForNumber(
        ...
        function () {
          ...
          return this.getRecord(address).then(
            function (record) {
              var preKeyProto =
                Internal.protobuf.PreKeyWhisperMessage.decode(buffer);
              if (!record) {
    ...
    doDecryptWhisperMessage: function (messageBytes, session) {
      ...
      var mac = messageBytes.slice(
        messageBytes.byteLength - 8,
      ...
      if (session === undefined) {
        return Promise.reject(
          new Error(
            "No session found to decrypt message from " +
              this.remoteAddress.toString()
          )
        );
      }
        ...
        .then(
          function (keys) {
            return this.storage
              .getIdentityKeyPair()
              .then(
                function (ourIdentityKey) {
                  var macInput = new Uint8Array(
                    messageProto.byteLength + 33 * 2 + 1
                  );
                  macInput.set(
                    new Uint8Array(
                  ...
                  macInput.set(
                    new Uint8Array(util.toArrayBuffer(ourIdentityKey.pubKey)),
                    33
                  );
                  macInput[33 * 2] = (3 << 4) | 3;
              ...
              .then(function () {
                return Internal.crypto.decrypt(
                  keys[0],
    ...
    fillMessageKeys: function (chain, counter) {
      ...
      byteArray[0] = 1;
      return Internal.crypto.sign(key, byteArray.buffer).then(
        function (mac) {
          byteArray[0] = 2;
          return Internal.crypto.sign(key, byteArray.buffer).then(
            function (key) {
    ...
    maybeStepRatchet: function (session, remoteKey, previousCounter) {
      ...
      return Promise.resolve()
        .then(
          function () {
            var previousRatchet =
              session[util.toString(ratchet.lastRemoteEphemeralKey)];
            if (previousRatchet !== undefined) {
              return this.fillMessageKeys(
                previousRatchet,
                previousCounter
              ).then(function () {
          ...
          function () {
            return this.calculateRatchet(session, remoteKey, false).then(
              function () {
                ...
                var previousRatchet = util.toString(
                  ratchet.ephemeralKeyPair.pubKey
                );
                if (session[previousRatchet] !== undefined) {
                  ratchet.previousCounter =
                    session[previousRatchet].chainKey.counter;
                  delete session[previousRatchet];
                }

                return Internal.crypto.createKeyPair().then(
                  function (keyPair) {
    ...
    getRemoteRegistrationId: function () {
      return Internal.SessionLock.queueJobForNumber(
        this.remoteAddress.toString(),
        function () {
          return this.getRecord(this.remoteAddress.toString()).then(function (
            record
          ) {
            if (record === undefined) {
              return undefined;
        ...
        function () {
          return this.getRecord(address).then(
            function (record) {
              if (
                record === undefined ||
                record.getOpenSession() === undefined
              ) {
                return;
              }
  ...
  libsignal.SessionCipher = function (storage, remoteAddress) {
    ...
    this.decryptPreKeyWhisperMessage =
      cipher.decryptPreKeyWhisperMessage.bind(cipher);
    ...
    this.closeOpenSessionForDevice =
      cipher.closeOpenSessionForDevice.bind(cipher);
    this.deleteAllSessionsForDevice =
      cipher.deleteAllSessionsForDevice.bind(cipher);
  };
};
